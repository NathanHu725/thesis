...
class RadiationConnection(Connection):
    def __init__(self, name, nodes, destination_node, travel_vars, distance):
        ...
        self.T_i = travel_vars['commuter_proportion'] 
        
        # Below are experiments of how to weight the commuter proportion
        #* ((np.log(3000000 / nodes[name].get_population())) + 1) #* 3 * (len(self.included_nodes)) / len(nodes.values())
